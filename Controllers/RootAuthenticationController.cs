...
namespace MeAnotoApi.Controllers {
	[Route("Api/Authentication/Root")]
	[ApiController]
	public class RootAuthenticationController : ControllerBase {
		private readonly UserManager<RootUser> _UserManager;
		private readonly RoleManager<IdentityRole> _RoleManager;
		private readonly IConfiguration _Configuration;
		public RootAuthenticationController(UserManager<RootUser> userManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration) {
		...
		public async Task<IActionResult> RegisterRoot([FromBody] RootRegisterModel model) {
			...
			RootUser user = model.Role switch {
				"Administrator" => new AdministratorUser() {
					Email = model.Email,
			...
			if (!result.Succeeded) {
				return this.BadRequest(new Response { Status = "Error", Message = "Creation failed" });
			}
